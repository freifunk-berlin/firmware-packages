...
--os.execute('/usr/bin/ws-mirror-server-cmd --port=7682 -m 99,'..name..','..value..' 127.0.0.1>/dev/null')
--os.execute('/usr/bin/ws-mirror-server-cmd --port=7681 -m 99,'..name..','..value..' 127.0.0.1>/dev/null')
local el = {}
el.id=99
el.name=name
el.group=group
	...
	if s.name == name then
		...
		if event == 'alarm' then
			if value=="1" then
				...
				--ws_ret=os.execute('/usr/bin/ws-mirror-server-cmd --port=7681 -m 99,'..name..','..value..','..group..','..comment..','..ontime..','..offtime..','..acktime..','..lastime..','..ack..' 127.0.0.1')
				local el = {}
				el.id=99
				el.name=name
				el.value=value
